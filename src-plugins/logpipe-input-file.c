...
char	*__LOGPIPE_INPUT_FILE_VERSION = "0.2.0" ;
...
struct TraceFile
{
	...
	struct timeval		alive_timestamp ;
	struct rb_node		alive_timestamp_rbnode ;
	
	struct timeval		deleted_timestamp ;
	struct rb_node		deleted_timestamp_rbnode ;
} ;
...
struct InputPluginContext
{
	...
	struct rb_root		alive_timestamp_rbtree ;
	struct rb_root		deleted_timestamp_rbtree ;
...
funcCompareRbTreeNodeEntry CompareAliveTimestampRbTreeNode ;
int CompareAliveTimestampRbTreeNode( void *pv1 , void *pv2 )
{
	struct TraceFile	*p_trace_file1 = (struct TraceFile *)pv1 ;
	struct TraceFile	*p_trace_file2 = (struct TraceFile *)pv2 ;
	
	if( p_trace_file1->alive_timestamp.tv_sec < p_trace_file2->alive_timestamp.tv_sec )
	{
		return -1;
	}
	else if( p_trace_file1->alive_timestamp.tv_sec > p_trace_file2->alive_timestamp.tv_sec )
	{
		return 1;
	}
	else
	{
		if( p_trace_file1->alive_timestamp.tv_usec < p_trace_file2->alive_timestamp.tv_usec )
		{
			return -1;
		}
		else if( p_trace_file1->alive_timestamp.tv_usec > p_trace_file2->alive_timestamp.tv_usec )
		{
			return 1;
		}
		else
...
LINK_RBTREENODE_ALLOWDUPLICATE( LinkTraceFileAliveTimestampTreeNode , struct InputPluginContext , alive_timestamp_rbtree , struct TraceFile , alive_timestamp_rbnode , CompareAliveTimestampRbTreeNode )
UNLINK_RBTREENODE( UnlinkTraceFileAliveTimestampTreeNode , struct InputPluginContext , alive_timestamp_rbtree , struct TraceFile , alive_timestamp_rbnode )

funcCompareRbTreeNodeEntry CompareDeletedTimestampRbTreeNode ;
int CompareDeletedTimestampRbTreeNode( void *pv1 , void *pv2 )
{
	struct TraceFile	*p_trace_file1 = (struct TraceFile *)pv1 ;
	struct TraceFile	*p_trace_file2 = (struct TraceFile *)pv2 ;
	
	if( p_trace_file1->deleted_timestamp.tv_sec < p_trace_file2->deleted_timestamp.tv_sec )
	{
		return -1;
	}
	else if( p_trace_file1->deleted_timestamp.tv_sec > p_trace_file2->deleted_timestamp.tv_sec )
	{
		return 1;
	}
	else
	{
		if( p_trace_file1->deleted_timestamp.tv_usec < p_trace_file2->deleted_timestamp.tv_usec )
		{
			return -1;
		}
		else if( p_trace_file1->deleted_timestamp.tv_usec > p_trace_file2->deleted_timestamp.tv_usec )
		{
			return 1;
		}
		else
...
LINK_RBTREENODE_ALLOWDUPLICATE( LinkTraceFileDeletedTimestampTreeNode , struct InputPluginContext , deleted_timestamp_rbtree , struct TraceFile , deleted_timestamp_rbnode , CompareDeletedTimestampRbTreeNode )
UNLINK_RBTREENODE( UnlinkTraceFileDeletedTimestampTreeNode , struct InputPluginContext , deleted_timestamp_rbtree , struct TraceFile , deleted_timestamp_rbnode )
...
static int RotatingFile( struct InputPluginContext *p_plugin_ctx , struct TraceFile *p_trace_file )
{
	...
	/* 组装转档后文件名 */
	snprintf( old_filename , sizeof(old_filename)-1 , "%s/%.*s" , p_trace_file->pathname , p_trace_file->filename_len , p_trace_file->filename );
	gettimeofday( & tv , NULL );
	memset( & tm , 0x00 , sizeof(struct tm) );
	localtime_r( & (tv.tv_sec) , & tm );
	memset( new_filename , 0x00 , sizeof(new_filename) );
	snprintf( new_filename , sizeof(new_filename)-1 , "%s/_%.*s-%04d%02d%02d_%02d%02d%02d_%06ld" , p_trace_file->pathname , p_trace_file->filename_len , p_trace_file->filename , tm.tm_year+1900 , tm.tm_mon+1 , tm.tm_mday , tm.tm_hour , tm.tm_min , tm.tm_sec , tv.tv_usec );
...
static int RemoveFileWatcher( struct LogpipeEnv *p_env , struct LogpipeInputPlugin *p_logpipe_input_plugin , struct InputPluginContext *p_plugin_ctx , struct TraceFile *p_trace_file )
{
	...
	UnlinkTraceFileAliveTimestampTreeNode( p_plugin_ctx , p_trace_file );
#if 0
	UnlinkTraceFileDeletedTimestampTreeNode( p_plugin_ctx , p_trace_file );
#endif
...
_GOTO_WRITEALLOUTPUTPLUGINS :
	...
	/* 如果启用大小转档，且文件有追加内容 */
	if( rotate_size > 0 && p_trace_file->trace_offset >= rotate_size )
	{
		/* 大小转档文件 */
...
static int AddFileWatcher( struct LogpipeEnv *p_env , struct LogpipeInputPlugin *p_logpipe_input_plugin , struct InputPluginContext *p_plugin_ctx , char *filename , int read_full_file_flag , int rotate_size , int max_append_count )
{
	...
	/* 添加文件变化监视器 */
	p_trace_file->inotify_file_wd = inotify_add_watch( p_plugin_ctx->inotify_fd , p_trace_file->path_filename , (uint32_t)(IN_MODIFY|IN_CLOSE_WRITE|IN_DELETE_SELF|IN_MOVE_SELF|IN_IGNORED|IN_Q_OVERFLOW) ) ;
	...
	if( nret )
	{
		ERRORLOG( "LinkTraceFileInotifyWdTreeNode[%s] failed , errno[%d]" , p_trace_file->path_filename , errno )
	...
	gettimeofday( & (p_trace_file->alive_timestamp) , NULL );
	LinkTraceFileAliveTimestampTreeNode( p_plugin_ctx , p_trace_file );
	
#if 0
	gettimeofday( & (p_trace_file->deleted_timestamp) , NULL );
	LinkTraceFileDeletedTimestampTreeNode( p_plugin_ctx , p_trace_file );
#endif
	
	/* 如果要发送存量文件内容 */
	if( read_full_file_flag == READ_FULL_FILE_FLAG )
	{
		return CheckFileOffset( p_env , p_logpipe_input_plugin, p_plugin_ctx , p_trace_file , rotate_size , max_append_count );
	}
	else
	{
		return 0;
...
int InitInputPluginContext( struct LogpipeEnv *p_env , struct LogpipeInputPlugin *p_logpipe_input_plugin , void *p_context )
{
	...
	p_plugin_ctx->inotify_path_wd = inotify_add_watch( p_plugin_ctx->inotify_fd , p_plugin_ctx->path , (uint32_t)(IN_CREATE|IN_MOVED_TO|IN_DELETE_SELF|IN_MOVE_SELF|IN_Q_OVERFLOW) );
...
int OnInputPluginEvent( struct LogpipeEnv *p_env , struct LogpipeInputPlugin *p_logpipe_input_plugin , void *p_context )
{
	...
	while( p_inotify_event < p_overflow_inotify_event )
	{
		...
		else if( p_inotify_event->wd == p_plugin_ctx->inotify_path_wd )
		{
			...
			{
				INFOLOG( "inotify IN_CREATE event , wd[%d] mask[0x%X] cookie[%d] len[%d] name[%.*s]" , p_inotify_event->wd , p_inotify_event->mask , p_inotify_event->cookie , p_inotify_event->len , p_inotify_event->len , p_inotify_event->name )
			...
			{
				INFOLOG( "inotify IN_MOVED_TO event , wd[%d] mask[0x%X] cookie[%d] len[%d] name[%.*s]" , p_inotify_event->wd , p_inotify_event->mask , p_inotify_event->cookie , p_inotify_event->len , p_inotify_event->len , p_inotify_event->name )
				
				nret = AddFileWatcher( p_env , p_logpipe_input_plugin , p_plugin_ctx , p_inotify_event->name , READ_APPEND_FILE_FLAG , p_plugin_ctx->rotate_size , p_plugin_ctx->max_append_count ) ;
				if( nret )
				{
					ERRORLOG( "IN_MOVED_TO AddFileWatcher[%s] failed , errno[%d]" , p_inotify_event->name , errno )
					return -1;
				}
			}
		...
		else
		{
			...
			else
			{
				...
				if( ( p_inotify_event->mask & IN_MODIFY ) || ( p_inotify_event->mask & IN_CLOSE_WRITE ) )
				{
					/* 合并处理文件变动事件 */
					UnlinkTraceFileAliveTimestampTreeNode( p_plugin_ctx , p_trace_file );
					p_trace_file->alive_timestamp.tv_sec = now.tv_sec ;
					p_trace_file->alive_timestamp.tv_usec = now.tv_usec ;
					LinkTraceFileAliveTimestampTreeNode( p_plugin_ctx , p_trace_file );
				}
				/* 如果发生 删除文件 或 移动文件 事件 */
				else if( ( p_inotify_event->mask & IN_DELETE_SELF ) || ( p_inotify_event->mask & IN_MOVE_SELF ) )
				{
					/* 合并处理文件删除事件 */
					UnlinkTraceFileDeletedTimestampTreeNode( p_plugin_ctx , p_trace_file );
					p_trace_file->deleted_timestamp.tv_sec = now.tv_sec ;
					p_trace_file->deleted_timestamp.tv_usec = now.tv_usec ;
					LinkTraceFileDeletedTimestampTreeNode( p_plugin_ctx , p_trace_file );
				}
				else
				{
	...
	/* 处理文件合并变动事件 */
	p = rb_last( & (p_plugin_ctx->alive_timestamp_rbtree) );
	while( p )
	{
		p_prev = rb_prev( p ) ;
		
		p_trace_file = container_of( p , struct TraceFile , alive_timestamp_rbnode ) ;
		if( p_trace_file->alive_timestamp.tv_sec != now.tv_sec || p_trace_file->alive_timestamp.tv_usec != now.tv_usec )
			break;
		
		INFOLOG( "inotify IN_MODIFY or IN_CLOSE_WRITE event , wd[%d] path_filename[%s]" , p_trace_file->inotify_file_wd , p_trace_file->path_filename )
	...
	/* 处理文件合并删除事件 */
	p = rb_last( & (p_plugin_ctx->deleted_timestamp_rbtree) );
	while( p )
	{
		p_prev = rb_prev( p ) ;
		
		p_trace_file = container_of( p , struct TraceFile , deleted_timestamp_rbnode ) ;
		if( p_trace_file->deleted_timestamp.tv_sec != now.tv_sec || p_trace_file->deleted_timestamp.tv_usec != now.tv_usec )
			break;
...
int AfterReadInputPlugin( struct LogpipeEnv *p_env , struct LogpipeInputPlugin *p_logpipe_input_plugin , void *p_context , uint32_t *p_file_offset , uint32_t *p_file_line )
{
	...
	INFOLOG( "AfterReadInputPlugin[%s] - trace_offset[%d]+=[%d] trace_line[%d]+=[%d]" , p_trace_file->path_filename , p_plugin_ctx->p_trace_file->trace_offset , (*p_file_offset) , p_trace_file->trace_line , (*p_file_line) )
