...
int	__LOGPIPE_OUTPUT_STDOUT_VERSION_0_1_0 = 1 ;
...
{
	printf( "BeforeWriteOutputPlugin - filename[%.*s]\n" , filename_len , filename );
...
{
	printf( "WriteOutputPlugin - file_offset[%"PRIu64"] file_line[%"PRIu64"] block_len[%"PRIu64"] block_buf[%.*s]\n" , file_offset , file_line , block_len , (int)block_len,block_buf );
...
{
	printf( "AfterWriteOutputPlugin - filename[%.*s]\n" , filename_len , filename );
